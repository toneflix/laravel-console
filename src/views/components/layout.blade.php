...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('visualconsole/assets/tailwind.output.css') }}" />
    <script src="https://unpkg.com/alpinejs@3.x.x/dist/cdn.min.js" defer></script>
    <script src="{{ asset('visualconsole/assets/init-alpine.js?v=') . time() }}"></script>
    <script src="{{ asset('visualconsole/assets/focus-trap.js?v=') . time() }}"></script>
    <style>
        [x-cloak] {
            display: none;
            visibility: hidden;
    ...
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen }" x-cloak>
        ...
        <aside class="z-20 hidden w-64 overflow-y-auto bg-white dark:bg-gray-800 md:block flex-shrink-0">
            <div class="py-4 text-gray-500 dark:text-gray-400">
                ...
                <ul>
                    @php
                        $routes = LaravelVisualConsole::routes();
                    @endphp
                    @foreach ($routes as $route)
                        <li class="relative px-6 py-3">
                            @if (url()->current() ===
                                route(config('laravel-visualconsole.route_prefix', 'lvc') . $route['to'], $route['params']))
                                <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg"
                                    aria-hidden="true"></span>
                            @endif
                            <a class="inline-flex items-center w-full text-sm font-semibold text-gray-800 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100"
                                href="{{ route(config('laravel-visualconsole.route_prefix', 'lvc') . $route['to'], $route['params']) }}">
                                <i class="ri-{{ $route['icon'] }}-line"></i>
                ...
                <ul>
                    <li class="relative px-6 py-3">
                        <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg"
                            aria-hidden="true"></span>
                        <a class="inline-flex items-center w-full text-sm font-semibold text-gray-800 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100"
                            href="{{ route(config('laravel-visualconsole.route_prefix', 'lvc') . '.console.user') }}">
                        ...
                        <li class="relative">
                            ...
                            <template x-if="isProfileMenuOpen">
                                <ul x-transition:leave="transition ease-in duration-150"
                                    ...
                                    <li class="flex">
                                        <form
                                            action="{{ route(config('laravel-visualconsole.route_prefix', 'lvc') . '.console.logout') }}"
